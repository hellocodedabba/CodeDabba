...
import { Repository, MoreThan, LessThanOrEqual, In, DataSource } from 'typeorm';
...
export class HackathonsService {
    ...
    async handleCron() {
        ...
        for (const round of rounds) {
            ...
            // any state (except judging/closed) -> upcoming if dates were shifted back
            else if (round.status === RoundStatus.ACTIVE && now < round.startDate) {
            ...
            if (statusChanged) {
                ...
                // Update hackathon status if first round becomes active
                if (round.status === RoundStatus.ACTIVE) {
                    ...
                    if (hackathon && hackathon.status === HackathonStatus.READY_FOR_ROUND_1) {
                        ...
                        await this.hackathonsRepository.save(hackathon);
                        console.log(`Hackathon ${hackathon.title} transitioned to ROUND_ACTIVE`);
                    }
                }
            }
        }
    ...
    async distributeTeamsToMentors(user: User, hackathonId: string) {
        ...
        if (mentors.length === 0) throw new BadRequestException('No mentors registered for this hackathon');
    ...
    async finalizeTeams(hackathonId: string) {
        return await this.dataSource.transaction(async (manager) => {
            ...
            // 4. Move Hackathon to approval_in_progress
            hackathon.status = HackathonStatus.APPROVAL_IN_PROGRESS;
